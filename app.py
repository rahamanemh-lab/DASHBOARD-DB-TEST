...
LAMBDA_NAME = "crypto-s3"  # ← nom exact de ta Lambda

REGION = "eu-north-1"
DB     = "crypto_datalake"
# Ton dossier de résultats Athena (celui que tu as mis dans la console Athena)
S3_OUT = "s3://mon-datalake-demo-2024/athena_results/"
TABLE = "crypto_prices_raw"

st.set_page_config(page_title="Test Graphe Athena", layout="wide")
st.title("🔎 Test graphe forcé (200 dernières lignes)")


st.sidebar.markdown("### ⚡ Rafraîchissement immédiat")
...
try:
    ...
    st.success("Connexion Athena OK")
...
# ---------- Sanity très court ----------
try:
    st.write("Date Athena (UTC):", run_sql_df("SELECT current_date AS today"))
except Exception as e:
    st.exception(e)
    st.stop()

# ---------- Préview + graphe FORCÉ ----------
st.subheader(f"Aperçu brut `{DB}.{TABLE}` (200 dernières lignes)")
try:
    sql = f"""
        SELECT
          dt,
          ...
          COALESCE(
          TRY(from_iso8601_timestamp(replace(timestamp,' ','T'))),
        ...
        """
    df = run_sql_df(sql)
    st.write("Colonnes brutes:", list(df.columns))
    st.write("Shape brut:", df.shape)
    st.dataframe(df.head(200), use_container_width=True)
except Exception as e:
    st.error("Erreur SELECT * (aperçu)")
    st.exception(e)
    st.stop()

df["price"] = pd.to_numeric(df["price"], errors="coerce")
df["ts"]    = pd.to_datetime(df["ts"], errors="coerce")
df = df.dropna(subset=["price","ts"]).sort_values(["ts","asset"])
df["t_minute"] = df["ts"].dt.floor("T")
pivot = (
    df.groupby(["t_minute","asset"], as_index=False)["price"].mean()
    .pivot(index="t_minute", columns="asset", values="price")
    .sort_index()
)
st.line_chart(pivot)
...
series["ts"]    = pd.to_datetime(series["ts"], errors="coerce", utc=True)

st.subheader("📈 200 lignes sans agrégation (UTC)")
chart = (
    alt.Chart(series)
    .mark_line(point=True)
    .encode(
        x=alt.X("ts:T", title="Timestamp (UTC)"),
        y=alt.Y("price:Q", title="Prix"),
        color=alt.Color("asset:N", title="Asset"),
        strokeDash=alt.StrokeDash("fiat:N", title="Fiat"),
        tooltip=["ts:T","asset","fiat","price"]
    )
    .interactive()
)
...
if st.sidebar.button("▶️ Ingestion immédiate (Lambda)"):
    try:
        lam = boto3.client("lambda", region_name=REGION)
        resp = lam.invoke(
            FunctionName=LAMBDA_NAME,
            InvocationType="RequestResponse",  # on attend la fin
            Payload=b"{}"
        )
        st.success("Lambda exécutée.")
        # Petite pause: écrire S3, puis déclarer la partition
        time.sleep(2)
    except Exception as e:
        st.error(f"Erreur appel Lambda: {e}")

if st.sidebar.button("🧭 Découvrir la dernière partition (MSCK REPAIR)"):
    try:
        run_sql_df("MSCK REPAIR TABLE crypto_datalake.crypto_prices_raw")
        st.success("MSCK REPAIR OK — partitions mises à jour.")
        time.sleep(1)
        st.rerun()
    except Exception as e:
        st.error(f"Erreur MSCK REPAIR : {e}")
